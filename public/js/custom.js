...
"use strict";	
    ...
    $('#view-plans').bind('click', function(e) {
        e.preventDefault();
        var target = this.hash;
	...
	if( !device.tablet() && !device.mobile() ) { //Load the animations if the device is not a mobile or tablet
		$('head').append('<link rel="stylesheet" type="text/css" media="screen" href="css/animate.css">');
	}
...
jQuery(function() {
    ...
    socket.on('connect', function(){
        ...
        socket.on('totalpixels', function (data) {
            ...
            if(data)
            {
                ...
                if(data.statistics)
                {
                    $("#total-hits-holder").show();
                    $('#total-hits').html(addCommas(data.statistics.hits));
                }
            } else{
                $("#total-hits-holder").hide();
            }
        });
    });
